...
class Posts extends Model
{
  ...
  public function all_posts_category($start, $limit)
  {
    $query =
      "SELECT * FROM posts JOIN categories on categories.id_category = posts.category_id JOIN users ON users.id_user = posts.user_id LIMIT $start, $limit";
    $result = mysqli_query($this->db, $query);
    return $this->convert_data($result);
  }
}

// "SELECT posts.id_posts, posts.content, posts.attachment_post, posts.tittle, categories.name_category, posts.user_id, users.full_name, users.avatar, pivot_posts_tags.post_id_pivot, pivot_posts_tags.tag_id_pivot, tags.name_tag FROM posts JOIN categories ON posts.category_id = categories.id_category JOIN users ON posts.user_id = users.id_user JOIN pivot_posts_tags ON pivot_posts_tags.post_id_pivot = posts.id_posts JOIN tags ON pivot_posts_tags.tag_id_pivot = tags.id_tag";